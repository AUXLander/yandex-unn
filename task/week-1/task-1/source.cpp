...
public:
    rW1T1() : Task(nullptr) {}
    explicit rW1T1(Test* const reference) : Task(reference) {}
};
...
struct Node
{
    ...
    struct NodeCounter
    {
        ...
        NodeCounter& set(const size_t Count, Node* const Pointer)
        {
            count = Count;
            pointer = Pointer;
    ...
    Node(int value, size_t cm, Node* const pmore, size_t cl, Node* const pless)
        : value(value), more(cm, pmore), less(cl, pless)
    {
    }
};
    ...
    if (length > 0)
    {
        ...
        while (stack.empty() == false)
        {
            ...
            for (auto it = nodes.rbegin(); it != nodes.rend(); ++it)
            {
                ...
                {
                    loc_max_more.set(it->less.count + 1, &(*it));
                }

                if ((it->value < top) && (it->more.count + 1 > loc_max_less.count))
                {
                    loc_max_less.set(it->more.count + 1, &(*it));
                }
            }

            auto& node = nodes.emplace_back(top, loc_max_more.count, loc_max_more.pointer,
                                                 loc_max_less.count, loc_max_less.pointer);

            if (node.more.count >= max_more.count)
            {
                max_more.set(node.more.count, &node);
            }

            if (node.less.count >= max_less.count)
            {
                max_less.set(node.less.count, &node);
            }
